...
# #### Get the keywords from DB for threat
...
def get_keywords_from_DB():
    ...
    df=dbc.fetch_data("select [MASTER_ID],[THREAT_KEYWORDS]  from [SAAB_ML_MASTER_ESCALATION_THREAT_DM] where [DOMAIN_ID]="+cfg.client_details['Domain_ID'])
    return df


# #### Assigning the threat flag for each call
...
# #### Storing the threat flag into DB

# In[7]:


def update_data_into_database(df):
    ...
    for index,row in df.iterrows():
        id_count+=1
        cursor.execute("UPDATE [dbo].[SAAB_ML_ADDITIONAL_RESULT_FT] SET [HAS_THREAT]=?, [UPDATED_DATE]=?, [UPDATED_BY]=? WHERE CALL_ID=? AND DOMAIN_ID=?",bool(row['HasThreat']),datetime.datetime.now(),cfg.client_details['UserName'], int(row['Call_ID']) ,int(cfg.client_details['Domain_ID']))
    conn.commit()
    cursor.close()
    conn.close()
...
def main():
    ...
    logging.debug("Fetching the presence of threat keywords per call")
    try:
        #Verify the keywords presence in the calls
        final_df = determine_threat_in_calls(preprocessed_data,df_master_topics)
        logging.debug("Threat calls determined")
    ...
    try:
        update_data_into_database(final_df)
        ...
        logging.debug("End of Threat analysis")
